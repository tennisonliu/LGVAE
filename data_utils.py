"""
PUT DATA PREPROCESSING SCRIPTS IN HERE

THE FUNCTION SHOULD RETURN PYTORCH TENSORS THAT CAN BE DIRECTLY LOADED INTO TENSORS
"""
...
protein2onehot_dict = {"ALA":0, "ARG":1, "ASN":2, "ASP":3, "CYS":4, "GLN":5, "GLU":6, "GLY":7, "HIS":8, "ILE":9, "LEU":10, "LYS":11, "MET":12, "PHE":13, "PRO":14, "SER":15, "THR":16, "TRP":17, "TYR":18, "VAL":19}
...
def construct_new_graph(p, is_padding = False, padding=500):
    ...
    if is_padding == True:
        for i in range(p.number_of_nodes(), padding):
            G.add_nodes_from([(i, {"features": np.zeros(20)})])
    # construct edge info
...
#
# A = nx.adjacency_matrix(q).todense() # numpy 2D matrix of shape (len(q), len(q))
#
